...
class QmlWidgetHelper(QObjectHelper):
    ...
    def initQmlWidgetHelper(self, source, documentModel=None):
    ...
    def checkInitQml(self):
        ...
        if hasattr(self, 'qml'):
            return False
        self.qml = QQuickWidget(util.qmlEngine(), self)
        ...
        if self._documentModel:
            self.qml.rootObject().setProperty('documentModel', self._documentModel)        
        self.onInitQml()
        return True
    ...
    def findItem(self, objectName, noerror=False):
        ...
        item = root = self.qml.rootObject()
        for partName in parts:
            item = item.findChild(QObject, partName)
    ...
    def focusItem(self, objectName):
        ...
        if not item.hasActiveFocus():
            ...
            if not self.isActiveWindow():
                ...
                if util.IS_TEST:
                    import conftest
                    pngPath = conftest.dumpWidget(self)
                else:
                    pngPath = None
    ...
    def keyClicks(self, objectName, s, resetFocus=True, returnToFinish=True):
        ...
        if returnToFinish:
            util.qtbot.keyClick(self.qml, Qt.Key_Enter) # only for TextInput?
        if resetFocus:
            self.resetFocus(objectName)
    ...
    def mouseClickItem(self, item, buttons=Qt.LeftButton, pos=None):
        if pos is None:
            rect = item.mapRectToDocument(QRectF(0, 0, item.width(), item.height())).toRect()
            pos = rect.center()
    ...
    def clickTimelineViewItem(self, objectName, cellText, column=0, modifiers=Qt.NoModifier):
        ...
        rect = item.mapRectToDocument(QRectF(x, y, w, h))
    ...
    def clickComboBoxItem(self, objectName, itemText):
